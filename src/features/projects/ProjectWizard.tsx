                ...
                {step === 0 && (
                  <div className="grid gap-3 md:grid-cols-2">
                    {scopeOptions.map((scope, index) => {
                      ...
                      return <button type="button" key={scope.id} onClick={() => selectScope(scope.id)} aria-pressed={active} className={`rv-focus group relative rounded-lg border p-5 text-left transition ${active ? "border-[var(--rv-terracotta)] bg-[rgba(198,93,71,0.06)] shadow-sm" : "border-[var(--rv-border)] bg-white hover:border-[var(--rv-ink-2)]"}`}><div className="flex items-start gap-4"><span className={`grid h-11 w-11 shrink-0 place-items-center rounded-md ${active ? "bg-[var(--rv-terracotta)] text-white" : "bg-[var(--rv-bg)] text-[var(--rv-ink)]"}`}><Icon size={20} /></span><div><h3 className="font-display text-xl">{scope.title}</h3><p className="mt-1 text-sm leading-6 text-[var(--rv-ink-2)]">{scope.description}</p></div></div>{active && <CheckCircle2 className="absolute right-3 top-3 text-[var(--rv-terracotta)]" size={18} />}</button>;
                    })}
                  </div>
                )}
